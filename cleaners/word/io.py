"""DOCX cleaning JSONL input/output boundary."""
...
def clean_word_file(input_file: Path, output_file: Path, report_file: Path) -> dict[str, Any]:
    records = read_jsonl(input_file.resolve())
    meta = next((record for record in records if record.get("record_type") == "document_meta"), {})
    cleaned, report = clean_word_records(records, metadata=meta.get("metadata", {}))
