...
这里的 V1.2 是 V1 固定评测题集的修订版本，不是产品 V2，也不是另一套数据库。固定题集通过只表示当前数据和边界内的回归结果，不代表开放问题或任意年报具有 100% 准确率。

完整案例、历史失败、纯召回限制和测试数字的解释见 [V1 问题案例与测试结果边界](docs/V1问题案例与测试结果边界.md)。

## 配置文件

GitHub 中提供的配置模板是项目根目录的 [.env.example](.env.example)。真实本地配置文件为项目根目录 `.env`，它已被 `.gitignore` 排除，因此不会上传到 GitHub。
